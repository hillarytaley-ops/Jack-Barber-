...
async function handleRequest(req, res) {
  ...
  try {
        ...
        try {
          ...
          if (payment) {
            if (payment.providerPaymentId) {
              booking.payIdRequestId = payment.providerPaymentId;
              bookings[0] = booking;
              await writeJSON('bookings.json', bookings);
            }
            if (payment.checkoutUrl) {
              return send(res, 200, {
                ...
                amount: price,
                payment: payment.instructions || null
              });
            }
            return send(res, 200, {
              ok: true,
              id: booking.id,
              amount: price,
              payment: payment.instructions || payment
            });
          }
        } catch (e) {
          if (getPayId()) {
    ...
    if (req.method === 'POST' && pathname === '/api/payments/checkout') {
      ...
      if (!paymentsEnabled()) return send(res, 503, { error: 'PayID payments are not configured yet.' });
      ...
      try {
        ...
        if (payment && payment.checkoutUrl) {
          return send(res, 200, { checkoutUrl: payment.checkoutUrl, payment: payment.instructions || null });
        }
        if (payment && payment.instructions) {
          return send(res, 200, { payment: payment.instructions });
        }
        return send(res, 503, { error: 'PayID is not configured.' });
      } catch (e) {
