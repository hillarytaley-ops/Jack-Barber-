...
const UPLOADS = path.join(ROOT, 'uploads', 'gallery');
...
function ensureData() {
  ['settings', 'bookings', 'transactions'].forEach(function (name) {
    const target = path.join(DATA_DIR, name + '.json');
    if (!fs.existsSync(target)) {
      fs.copyFileSync(path.join(DATA_DIR, name + '.default.json'), target);
    }
  });
}
...
const server = http.createServer(async function (req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && pathname === '/api/admin/login') {
      ...
      return send(res, 200, { token, username: admin.username });
    }
    ...
    if (req.method === 'GET' && pathname === '/api/admin/reports') {
      return send(res, 200, buildReport(url.searchParams.get('period') || 'daily'));
    }
